    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="bg-obsidian border border-hairline rounded-2xl p-6 shadow-sm">
        ...
        <div className="mt-6 pt-4 border-t border-hairline flex justify-end">
          <button
            type="button"
            onClick={useAgentStore.getState().logout}
            className="px-4 py-2 rounded-xl text-[11px] font-bold uppercase tracking-wider text-red-400 border border-red-900/30 hover:bg-red-950/30 transition-colors"
          >
            Log Out
          </button>
        </div>
                    ...
                    <div className="px-4 pb-4 pt-2 border-t border-hairline bg-obsidian space-y-3">
                      ...
                      <div className="flex flex-col gap-4 pt-1">
                        {group.fields.map((field) => {
                          if (field.key === "GOOGLE_REFRESH_TOKEN") {
                            return (
                              <div key={field.key} className="mt-4 pt-4 border-t border-hairline">
                                <label className="type-mono text-[10px] text-muted whitespace-nowrap mb-2 block">
                                  Google Authorization
                                </label>
                                <a
                                  href="http://127.0.0.1:8000/api/auth/google/login"
                                  className="w-full flex items-center justify-center gap-3 bg-white text-black rounded-lg py-3 px-4 font-semibold hover:bg-gray-100 transition-colors shadow-sm"
                                ...
                                {field.set && (
                                  <p className="mt-2 text-[10px] leading-snug text-ok text-center font-bold">
                                    ✓ Authorized
                                  </p>
                                )}
                              </div>
